...
internal sealed class Win11SettingsForm : Form, IMessageFilter, ISettingsWindow
{
    ...
    private void BuildPages()
    {
        ...
        AddPageGrouped("\uE700", "操作面板", "左下角操作面板的按钮、透明度和外观。", new string[][]
        {
            ...
            new string[] { "Spec Board", "SpecBoardTransparencyOverridePercent", "SpecBoardLeftDockTabCenterY", "LeftDockOutsideClickCollapseEnabled", "SpecBoardAutoHideSeconds", "SpecBoardLedgerPath", "SpecBoardManagerWidth", "SpecBoardManagerHeight", "SpecBoardManagerDangerZoneRequiresTypedConfirm" },
            ...
            new string[] { "系统日记看板", "SystemDayBoardSmoothingEnabled", "SystemDayBoardTransparencyOverridePercent", "SystemDayBoardScaleOverridePercent", "SystemDayBoardLeftDockTabCenterY", "SystemDayBoardAutoHideSeconds" },
            new string[] { "字幕看板", "CaptionsBoardTransparencyOverridePercent", "CaptionsBoardScaleOverridePercent", "CaptionsBoardLeftDockTabCenterY", "CaptionsBoardAutoHideSeconds" },
    ...
    private static bool IsLeftDockTabCenterSetting(string propertyName)
    {
        return string.Equals(propertyName, "SpecBoardLeftDockTabCenterY", StringComparison.Ordinal) ||
            ...
            string.Equals(propertyName, "CaptionsBoardLeftDockTabCenterY", StringComparison.Ordinal);
    }
    ...
    private void VerifySelfTest()
    {
        string[] required = new string[]
        {
            ...
            "CaptionsBoardLeftDockTabCenterY",
            "CaptionsBoardAutoHideSeconds",
            "CaptionsBoardTransparencyOverridePercent",
            "CaptionsBoardScaleOverridePercent",
            "RightTileAutoArrangeEnabled",
    ...
    private void VerifyLeftDockTabCenterEditorPolicy()
    {
        ...
        normalized.CaptionsBoardLeftDockTabCenterY = -90;
        ...
        if (normalized.SpecBoardLeftDockTabCenterY != WidgetSettings.AutoLeftDockTabCenterY ||
            ...
            normalized.CaptionsBoardLeftDockTabCenterY != WidgetSettings.AutoLeftDockTabCenterY)
        {
    ...
    private static Dictionary<string, string> CreateSettingsUiBindingExemptions()
    {
        ...
        AddSettingsUiBindingExemptions(exemptions, "compatibility-only dock flags; the canonical visible topology always contains all eight left-edge tabs", new string[]
        {
    ...
    private static readonly Dictionary<string, NumericRange> NumericRanges = new Dictionary<string, NumericRange>(StringComparer.Ordinal)
    {
        ...
        { "CaptionsBoardLeftDockTabCenterY", new NumericRange(WidgetSettings.AutoLeftDockTabCenterY, 1000000) },
        { "CaptionsBoardAutoHideSeconds", new NumericRange(WidgetSettings.MinCaptionsBoardAutoHideSeconds, WidgetSettings.MaxCaptionsBoardAutoHideSeconds) },
        ...
        { "CaptionsBoardScaleOverridePercent", new NumericRange(WidgetSettings.MinWindowScaleOverridePercent, WidgetSettings.MaxWindowScaleOverridePercent) },
        ...
        { "CaptionsBoardTransparencyOverridePercent", new NumericRange(WidgetSettings.MinWindowTransparencyOverridePercent, WidgetSettings.MaxWindowTransparencyOverridePercent) },
    ...
    private static readonly Dictionary<string, string> SettingTitles = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        { "CaptionOverlayHoverAutoHideEnabled", "鼠标移上时自动淡出" },
        ...
        { "CaptionsBoardScaleOverridePercent", "字幕看板缩放覆盖" },
        ...
        { "CaptionsBoardTransparencyOverridePercent", "字幕看板整体透明度覆盖" },
    ...
    private static readonly Dictionary<string, string> SettingHints = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        { "CaptionOverlayEnabled", "由本程序自己绘制字幕条：从翻译器主窗口实时读取它正在显示的原文与译文，用本程序的排版画一条点击穿透的横幅。翻译器因此不需要任何可见窗口——它自己的覆盖窗只能靠点按钮打开、每次重启就没了，样式也只有 setting.json 里那几项。面板在翻译器没输出时完全不显示。" },
        ...
        { "CaptionOverlayHoverAutoHideEnabled", "打开后，鼠标移到字幕横条上时它会淡到几乎看不见（透明度 90%），移开就恢复。用来临时看清或点到被字幕盖住的东西，而不必真的把它关掉。字幕条本身全程点击穿透，所以悬停是靠按 250ms 比对指针位置与横条矩形判断的，不是靠鼠标消息；编辑模式下不淡出——正在拖的东西必须看得见。字幕看板里的「避让」按钮改的就是这一项。" },
        ...
        { "CaptionsBoardAutoHideSeconds", "范围 0-600 秒；0 表示展开后不自动收回。" },
        ...
        { "CaptionsBoardScaleOverridePercent", "−1 = 跟随全局分辨率兼容缩放；40–200 只覆盖字幕看板及其停靠标签。" },
        ...
        { "CaptionsBoardTransparencyOverridePercent", "−1 = 跟随全局整体透明度；0–90 只覆盖字幕看板及其停靠标签。" },
