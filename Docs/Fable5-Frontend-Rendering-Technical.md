...
> **历史快照（SUPERSEDED）**
>
> 本文冻结于 `1.0.4.18` 的旧多悬浮窗渲染实现；截至 `1.0.6.29`，其中的可见窗口清单、经典主窗口、独立 Radar、渲染命令、设置和验收入口均可能已经移除。本文只供历史追溯，**禁止作为现行实现、修改或验收依据**。当前可见拓扑与分层运行时以 [`Performance-And-Window-Runtime.md`](Performance-And-Window-Runtime.md) 为 SSOT，各模块细节以现行 `*-Architecture.md` 为准，当前命令以 [`README.md`](../README.md) 和机器接口索引为准。

快照版本：`1.0.4.18`。本文记录当时的前端渲染管线、变体系统和查验约定；下文“必读”“禁改”“必须”等措辞只描述该快照版本的历史要求，对当前版本不具约束力。
