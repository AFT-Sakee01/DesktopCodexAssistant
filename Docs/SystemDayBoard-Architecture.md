...
顶部六张摘要卡使用独立于图表的可读性字号层级：`SummaryTitleFontPixels = 8.8`、`SummaryDetailFontPixels = 8.8`、`SummaryNameFontPixels = 9.2`、`SummaryValueFontPixels = 11.0`；看板标题为 `S(13)`，图表标签为 `S(8)`。时长、电量、功耗和温度作为主数值加粗显示；预计充放电时间与热区名称保留各自的完整可用宽度，并在极端长文本时使用省略号，避免侵入相邻卡片。

底部左侧只保留一枚绿色范围按钮和一枚红色“关闭”，与其余六个左缘 board 的“主操作 + 关闭”结构一致。范围按钮显示当前范围并按“今天 → 24 小时 → 近一周”循环切换，仍可访问全部三种投影；按钮统一使用实测字体宽度、42 逻辑像素最小宽度和 4 像素间距，状态摘要在关闭按钮后再留 5 像素；顶部不再绘制重复关闭符号。
