...
公网 IPv4 后台请求仍只使用 `api.ipify.org`，并在提交快照前校验返回值必须是 IPv4 地址；IPv6 或非 IP 文本不会写入 `PublicIp`。显示层与采集层分离：标题栏公网字段优先从当前网卡的可公开路由 IPv6 中取一个压缩短显，排除 `fc00::/7` ULA；没有可用 IPv6 时再回退到 `PublicIp` 的 IPv4。`IP4` / `IP6` 行只显示第一个地址，剩余地址折叠为 `+n`；如果第一个地址按当前行宽能完整显示，就不显示第二个地址，宽度不足时才压缩第一个地址。完整地址列表仍保留在快照中供判断网络身份。
...
如果云服务处于检测中，网络状态文字右侧显示黄色 `云服务测试中`。如果存在一个红色或橙色云服务，网络状态文字右侧先显示服务名加 `!`，下一次刷新显示失败层级或原因加 `!`，例如 `AWS!` / `拒绝访问!`。服务名映射为 Cloudflare、Akamai、Github、AWS、Azure、Google；Google Cloud 错误显示为 `Google!`。公网文字仍使用固定右侧区域绘制，不参与云服务提示布局。

如果同时存在多个红色或橙色云服务，网络状态文字右侧按方块从左到右轮换，每个服务都按“服务名!”、“原因!”顺序随 UI 刷新切换。红色表示无法连接，橙色表示状态异常。

Classic 布局的 DNS 告警栏位于独立右对齐覆盖层，垂直位置对齐 `GFW` 行并在最右下角显示；它不参与 `GFW` 或 `IP6` 行宽度计算，不会让这两行预留、收窄或重排。IP 行只显示第一个地址和 `+n`，避免多个地址挤占行宽。DNS 行保留网卡返回的 DNS 优先级顺序，错误状态只改变颜色，不把报错项提前。DNS 没有错误时显示绿色 `DNS正常`；任一 DNS 为 `Problem`、`Hijacked` 或 `Unavailable` 时，按 DNS 行从左到右的原始优先级顺序随网络窗口绘制节拍轮换完整提示，并附带已有 `Reason` 的短原因，例如 `DNS污染`、`DNS仅TCP`、`DNS返回SERVFAIL`、`DNS无地址`、`DNSNX验证失败` 或 `DNS地址无效`，不显示 `DNS1` / `DNS2` 编号。多个 DNS 具有相同异常文字和状态时，告警文字追加紧凑 DNS 地址后缀，例如 `DNS仅TCP@1.1.1.1`，避免无编号后看起来一直停在同一条。DNS 行和 DNS 告警栏的正常状态统一使用标准绿色，不再使用淡绿色；告警颜色复用 DNS 行对应状态色，不额外发起 DNS 探测。
