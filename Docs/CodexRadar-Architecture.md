...
`ApplyQuotaSnapshot()` 是账户打戳的唯一保证点：任何进入它的 Codex 快照都会先按当前身份补上 `AccountKey` 与 `AccountLabel`，缓存身份仍未知时就地解析一次。`ReadQuotaSnapshot()` 另外在读取时打戳，用于缓存归属校验与 `codex-radar-cache.ini` 回写；只在读取处打戳不够，provider usage 调度器会绕过该路径直接 apply 自己的快照。`codex-radar-cache.ini` 写入 `AccountKey=`；读回时账户不匹配的缓存直接降级为 default，不作为当前余额展示。
...
`CodexQuotaHistoryStore` 仅接收 `ApplyQuotaSnapshot()` 已接受且允许记录 decision 的 Codex family 快照，并且只接收**重置保护改写之前**的那份读数：`ApplyQuotaSnapshot()` 先用 `CaptureAcceptedQuotaForHistory()` 克隆已接受快照，再让 `ApplyQuotaResetProtections()` 原地把展示快照改写成 100；`RecordAcceptedQuotaHistory()` 是写入本存储的唯一入口，只接受这份已接受读数。保护值继续供右侧 tile、provider 缓存与 `codex-quota.ini` 回退使用，但不得进入历史——`Record()` 会把强制的 44 → 100 读成一次重置事件，而 `ForceWeeklyQuotaToFull()` 同时清掉的 `weekly_reset_known` 还会让下一次真实重置失去判定自然重置所需的 anchor。`RunQuotaHistoryProtectionSelfTest()`（`--test-layout`）守这条不变量。每行带 `account_key`，`Record()` 的重置分类与采样间隔过滤只与**同账户**的上一条比较，`GetSnapshot(nowUtc, accountKey)` 只投影该账户的行；在能解析出当前账户的机器上，`Load()` 会丢弃账户不可归属（`account_key` 为 `unknown`）的历史行并立即重写文件——这类行无法事后补归属，留在 `unknown` 桶里看板也永远看不到；解析不出任何账户时不丢弃，因为此时看板本身也跑在 `unknown` 键上。内存立即更新，磁盘由 15 秒 ThreadPool timer 批量写入；普通样本至少间隔 15 分钟，或周余量变化达到 3%，周余量回升达到 5% 时立即登记。旧 reset anchor 前 15 分钟至后 6 小时内的回升标记为自然重置；重置卡计数同时减少时标记为重置卡；其余标记为硬重置。每 6 小时和 owner 退出时原子裁剪到最近 7 天、最多 2048 行。

### 7.7 公共 Radar 重置事件与额外重置提醒

公共 Radar 的 RSS 重置事件是**账户无关**的全局信号，所以它的去重状态与重置保护窗口挂在 `RadarFamilyRuntimeState.Protection`（family 级），不在按账户换出换入的 `QuotaRuntimeState` 内。`LoadQuotaResetState()` 在构造期从 `quota-reset-state.ini` 恢复 `LastRadarResetEventId` / `LastRadarResetEventUtc` / `LastRadarProtectedResetEventId`，这份状态必须跨越每一次账户换轨存活——它一旦被换轨丢弃，重启就等同于冷启动。

`IsNewRadarResetEvent()` 是唯一判定入口，纯函数、可测：

- 事件 key 等于 `LastRadarProtectedResetEventId` 时一律不再触发，这是最强的去重信号。
- `newerThanKnownEvent` 只在**已有**上一次事件时间时才成立。没有历史时间时任何时间戳都“新于 `DateTime.MinValue`”，冷启动必须改走新鲜度判断。
- 冷启动只接受 `RadarResetEventFreshnessHours = 36` 小时内的事件；RSS 条目长期保留，数月前的旧条目不构成“刚发生过重置”的证据。
- 同 id 但从未保护成功（活化被打断）时，只在仍新鲜的窗口内重试。
- 只有 id、没有时间的事件仍可触发一次，之后按 id 去重。

`RunRadarResetEventDetectionSelfTest()`（`--test-layout`）覆盖以上全部边界。
