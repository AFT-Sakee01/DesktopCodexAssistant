...
- 右侧状态：单行 `Rader` / `OpenAI` / `DeepSeek` / `Claude` API 摘要、网页数据标签（例如 `7.2_pm_2`）以及 Model IQ 更新状态，格式为 `已更新/HH:mm` 或 `未更新/HH:mm`
...
2. 处理网络可用性失效标记。
3. 判断本地额度是否到期。
4. 判断北京时间日界线驱动的 Radar、Claude 健康检查、Claude Code 用量检查和 DeepSeek 余额检查是否到期；五阶段连接诊断当前停用。
5. 根据设置计算窗口尺寸和防烧屏微位移。
...
ChatGPT usage provider 读取 `used_percent` 和 `reset_at` / `resets_at`，并把 provider 快照保留约 15 分钟；过期后即使上一次 provider 成功，也允许 session fallback 更新显示，避免服务端接口不可用时长期冻结旧值。session JSONL 回退中读取 `event_msg -> token_count -> rate_limits`。`primary` 通常对应 5 小时窗口，`secondary` 通常对应周窗口；如果存在 `window_minutes`，以是否小于等于 300 分钟重新判断。
...
每次额度读取完成并执行消耗环判定后，会向 `%LOCALAPPDATA%\DesktopCodexAssistant\quota-decision-history.jsonl` 追加一条 JSONL 诊断记录。记录中的 `*_balance_percent` 是最终显示余额，`*_raw_balance_percent` 是本次读取原始余额，`*_consumption_ring_percent` 是实际露出的消耗尾段，`*_consumption_baseline_percent` 是绘制在当前余额环下方的完整基线环，`reason` 说明为什么保留、重置或更新基线。日志不在 `DrawQuotaRow`、hover 动画或 layered-window 重绘路径写入，避免绘制帧造成写放大；记录器使用 15 秒或 32 KiB 批量落盘，启动和每 6 小时按约 48 小时窗口清理旧行。
...
当前版本把旧的竖向 `Rader`、`Claude`、`ChatGPT` 服务健康面板隐藏，`ServiceHealthPanelEnabled = false` 仍阻止该面板占用布局宽度；相关绘制函数保留在源码中，作为回滚路径，不删除。`1.0.3.47` 起，`ServiceHealthProbeEnabled = true` 恢复 Rader/Claude 服务状态刷新；`1.0.3.49` 起，配置了 DeepSeek API key 时，DeepSeek 余额接口失败、不可用或余额不足也进入同一个 API 摘要候选。`1.0.3.54` 起，五阶段连接诊断通过 `CodexConnectionFlowEnabled = false` 停止调度，网络变化、恢复和操作面板强制刷新都不会再启动网络/DNS/隧道/OpenAI/本地 Codex 五段请求；旧绘制函数和快照结构仅保留为回滚代码。`1.0.3.61` 起，当前软件为 `CLAUDE` 时，Claude Code 用量接口的未登录、鉴权失败、限流、不可达或解析失败也加入同一 API 摘要候选；公开 Claude 状态页仍独立保留。当前实际使用的 `EvenRow` 右侧状态格绘制 API 摘要、网页数据标签和更新时间：正常时显示绿色 `API无异常`，异常时按网络窗口云服务告警的模式在异常 API 名称和错误原因之间轮播，并按异常级别变色。网页数据标签和更新时间复用同一更新颜色，更新请求闪烁时一起闪烁。OpenAI 项在五阶段诊断停用时只消费服务健康兜底状态，不再读取旧连接快照。
...
- `EvenGrid`（`Core/CodexRadarForm.EvenGrid.cs`）：上方一行六等分单元格（时间效率、Token 效率、5 小时额度、周额度、IQ、额度雷达），下方一条满宽状态带三等分（社区体感评分、五阶段连接点+摘要文字、Model IQ 更新时间），中间用一条细横线分隔。
...
`NetworkChange` 回调只设置服务健康和 DeepSeek 余额失效标记，不在系统事件线程执行网络检查。个人额度刷新只通过 `RequestSelectedQuotaUsageRefresh` 排队当前有效且正在运行的软件 provider，不再同时触碰 Codex provider 和 Claude Code usage 队列。下一次 UI 调度统一更新服务健康摘要；五阶段连接诊断当前停用，网络变化不会再启动对应的网络/DNS/隧道/OpenAI/本地 Codex 探测。旧三行服务面板隐藏时不会恢复三行绘制。

## 8. Model IQ 与效率

### 8.1 IQ 环

默认任务总数为 10；网站 `score` 公式为 `passed / valid_tasks * 150`，所以每通过 1 题增加 15 分，100 分对应数学上的 6.67/10，当前实际绿色基准点为 7/10=105，6/10=90 为黄色边界。绝对通过数基准默认为 7/10。IQ 基准可单独设置为绝对通过数、近 7 日平均、近 30 日平均或全记录平均；记录数量不足指定窗口时临时退回到全记录平均，默认使用全记录平均。旧配置中仍处在历史默认 8/12 的用户会在 Version 40 迁移，10 题切换时仍处在 7/12 默认值的用户保持 7/10；上一版 Version 43 自动写下的 6/10 会在 Version 44 迁移到 7/10。手动改成其他值的配置保持不动，超过 10 的通过数会被夹到 10。旧缓存或旧网页回退数据若仍带 12 题有效任务，会按比例折算为 10 题口径后再参与显示和平均基准计算。

- 圆心：网站 `score` 四舍五入后的整数，不显示 `%`，允许超过 100
- 常态区：通过网页 `model-iq-band-label` 读取，例如 `90-110常态区`；缺失时才使用 90-110 作为离线保护
- 绿色基底：已知数据时先绘制一整圈绿色基准环
- `score` 低于常态区：红色从 12 点方向逆时针覆盖低于下限的不足部分
- `score` 落在常态区：只显示绿色基准满环
- `score` 高于常态区：金色从 12 点方向顺时针覆盖高于上限的超出部分
- 右侧文字：低于网站常态区显示 `降智`，落在网站常态区内（含边界）显示 `常态`，高于网站常态区显示 `增智`

如果网站只给 `passed / valid_tasks` 比率，会先按网站 150 分制换算 `score`，再根据有效任务数四舍五入反推通过数。
...
旧五阶段连接流程（网络、DNS、隧道、OpenAI、Codex）保留源码但停用：`CodexConnectionFlowEnabled = false` 会阻止 timer、网络变化、恢复和强制刷新启动该诊断；`DrawEvenLayoutConnectionSummary` 和 `DrawCodexConnectionFlowVertical` 在禁用时直接返回。失败请求不会覆盖更新时间；它只在 IQ 环实际接收新快照时更新。
...
- 请求成功且包含 `model_iq` 或首页 HTML 成功补齐所选模型：更新 IQ、效率、数据日期、刷新时间，并标记刷新成功。
...
- 启用层请求成功但缺少 `model_iq`：服务状态为 `Incomplete`，保留旧 IQ/效率，刷新时间更新并标记失败。
