# Radar Data And Adaptation Hardening SPEC (Consolidated)

> 状态（2026-07-11）：应执行侧反馈本文件过长，已拆分为两份可并行实施的 SPEC——`Codex-RadarCodexDataHardening-SPEC-v1.0.5.03-20260711-103509.md`（Codex 侧，Phase A0-A10）与 `Codex-RadarClaudeAdaptationAndClockLadder-SPEC-v1.0.5.03-20260711-103509.md`（Claude 适配 + 时钟，含已批准的 C2）。请以拆分版为准，本文件仅供追溯。

## Metadata

- Document: `Codex-RadarHardeningConsolidated-SPEC-v1.0.5.03-20260711-094500.md`
- Generated model: Claude Fable 5
- Timestamp local: `2026-07-11T09:45:00+09:00`
- Timezone: `Asia/Tokyo (+09:00)`
- Current version: `1.0.5.03`
- Target implementation version: `1.0.5.04`
- Status: draft（Phase C2 为待批准项，批准前跳过；其余全部可实施）
- Consolidates:
  - `Codex-CodexRadarModelCatalogHardening-SPEC-v1.0.5.01-20260710-232034.md`（未执行 → 本文件第一部分）
  - `Codex-RadarModelLifecycleAdaptation-SPEC-v1.0.5.01-20260711-081242.md`（未执行 → 本文件第二部分）
  - `Codex-RadarClockDialConsolidation-SPEC-v1.0.5.01-20260710-235413.md` 执行后（1.0.5.03）追加的 V4/V5 遗留项 → 本文件第三部分

## Goal

一次实施完成 Radar 数据层与适配层的全部待办：目录逻辑修复、缓存签名保真（绿点跨重启）、额度重置与幻影池判别（确定性算术，无等待兜底）、站点模型增减/改名/重排适配、时钟 "n" 后缀徽标兼容，以及待批准的状态色阶梯调整。各部分约束沿用来源 SPEC 原文。

## 实施顺序

Phase A0 → A1-A8（目录+缓存+重置判别）→ B0 → B1-B3（模型增减适配）→ C1（→ C2 若已批准）→ D（版本与文档同步）→ 统一验证矩阵 → 正式部署。
...
## 第二部分：模型增减适配（原 RadarModelLifecycleAdaptation SPEC；本部分 F1-F3 编号独立于第一部分）
...
## 第三部分：时钟表盘遗留项（RadarClockDialConsolidation 已于 1.0.5.03 执行；以下两项为执行后追加，尚未实现）
...
### Phase C2: 状态色阶梯整体后移一个窗口（V4，待用户批准；批准前跳过本 Phase）
...
实现通过后版本从 `1.0.5.03` 提升到 `1.0.5.04`，同步：

- `Core/ProductIdentity.cs`、`AGENTS.md`
- `Docs/Interfaces/INTERFACE_INDEX.jsonl`：目录/缓存/适配条目按第一、二部分要求更新；若 Phase C1/C2 实施则同步 `internal_api.radar_clock_dial`
- `Docs/Fable5-Data-Sources-And-Caching-Technical.md`：缓存签名持久化与新键说明
- `Docs/CodexRadar-Architecture.md` / `Docs/Codex-ClaudeRadar-Architecture.md`：已知限制条目（第一部分 Phase A8 与第二部分 Phase B3）
- `Docs/Maintenance/CHANGELOG.jsonl` change + deploy 记录
- `Docs/Technical/INDEX.jsonl`：登记本合并 SPEC 与对应 GoalSpec；三份来源 SPEC 标注"已并入/已执行"状态
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\Build-Arm64.ps1 -OutputPath .\_build\DesktopCodexAssistant-arm64-radar-hardening-consolidated-v1.0.5.04.exe -Platform arm64
.\_build\DesktopCodexAssistant-arm64-radar-hardening-consolidated-v1.0.5.04.exe --test
.\_build\DesktopCodexAssistant-arm64-radar-hardening-consolidated-v1.0.5.04.exe --test-layout
.\_build\DesktopCodexAssistant-arm64-radar-hardening-consolidated-v1.0.5.04.exe --test-settings-bindings
.\_build\DesktopCodexAssistant-arm64-radar-hardening-consolidated-v1.0.5.04.exe --test-radar-display-lifecycle --iterations 100
```
...
| 适配F1 完整性门禁 | 三个 MergeAndSave 调用点显式传完整性实参、HTML 恒 false；"不完整目录不衰减"自测通过 |
| 适配F1 定向处理 | 选中模型下架通知含"当前选中"；Deleted+auto-switch 开启时即时触发切换评估 |
| 适配F1 通知合并 | ≥4 事件合并为摘要通知，分类计数自测通过 |
| 适配F2 候选过滤 | HistoricalOnly/disabled/deleted 不被自动切换选中，自测通过 |
| 适配F2 改名跟随 | 三场景自测通过；用户自定义名不被覆盖 |
| 适配F2 重排跟随 | 完整目录后 sort_order 无重复且与站点一致；10 列旧行兼容 |
| 适配F3 文档 | 两份架构文档出现已知限制条目 |
| C1 n 后缀徽标 | HasSecondRunSuffix 识别 n 系后缀，自测断言通过 |
| C2 配色阶梯（若批准） | ComputePhase 新档位真值表自测通过，渲染样本按新基准重截 |
| 发布 | 版本 `1.0.5.04` 三方（Release/D/E）SHA256 与版本一致，D 实例重启后 Responding |
