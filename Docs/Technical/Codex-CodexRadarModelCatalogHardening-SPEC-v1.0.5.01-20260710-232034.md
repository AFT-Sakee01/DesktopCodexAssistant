# Codex Radar Model Catalog And Cache Hardening SPEC

> 状态（2026-07-11）：本 SPEC 未单独执行，全部内容已并入合并版 `Codex-RadarHardeningConsolidated-SPEC-v1.0.5.03-20260711-094500.md`（第一部分，Phase A0-A8）。请以合并版为准，本文件仅供追溯。

## Metadata

- Document: `Codex-CodexRadarModelCatalogHardening-SPEC-v1.0.5.01-20260710-232034.md`
- Generated model: Claude Fable 5
- Timestamp UTC: `2026-07-10T14:20:34Z`
- Timestamp local: `2026-07-10T23:20:34+09:00`
- Timezone: `Asia/Tokyo (+09:00)`
- Current version: `1.0.5.01`
- Target implementation version: `1.0.5.02`
- Status: draft
- Source: Codex 模块 review（GPT-5.6 目录换代 + 速蹬倒计时 + 低风险复用重构改动面）
- Scope extension log:
  - 2026-07-11 用户批准将 Radar 缓存机制审查（绿点跨重启重置等 F6-F9）的修复方案与检查机制并入本 SPEC
  - 2026-07-11 "站点模型增减适配"审查按用户指示独立成篇：`Codex-RadarModelLifecycleAdaptation-SPEC-v1.0.5.01-20260711-081242.md`（本 SPEC 不再包含该范围）

## Goal

修复 review 中确认的目录层 2 个逻辑缺陷、1 个观感缺陷，以及缓存层 3 个数据保真缺陷（含已实证的"绿点跨重启重置"），并把未来兼容性弱点与缓存既有取舍落为已知限制文档。站点模型增减适配已拆出为独立 SPEC。不改变速蹬倒计时视觉、额度环渲染节奏、缓存文件的整体格式（只新增键，不改既有键语义）和发布策略。

明确排除项（本 SPEC 不处理）：

- 速蹬窗口 `closed_at` 未知时环文字缺少"速蹬"字样兜底（review 项 P2）：用户决定不处理。
- `DrawEvenRowSpeedWindowCountdownDial` 中未使用的 `heroSuffix` 局部变量：`SplitEvenRowStatusHeroLabel` 的 out 参数无法省略，纯 lint，不动。
- `ApplyRadarClockAutoSwitchIfNeeded` 的 Load→改一键→Save 整文件回写竞态面：既有架构问题，1.0.5.01 已用所有权门缓解 Claude 键一例，本次不扩大范围。
...
确认无其他代理在改同一批文件，且版本仍为 `1.0.5.01`：

```powershell
rg -n "Version = \"1\.0\.5\.01\"" Core\ProductIdentity.cs
...
### Phase 1: 目录重复记录合并改为"较新记录整体为准"（F1）
...
### Phase 2: 默认模型迁移在 Normalize 后补跑（F2）
...
### Phase 3: GetDisplayLabel family 段首字母大写（F3）
...
### Phase 5: 空 key 前缀哨兵 + CheckedAt 持久化（F7）
...
验收：自测断言"WindowKnown=false 的快照 Save→Load 往返后 WindowKnown 仍为 false"；结合 Phase 4 签名持久化后，该字段不再参与重启误判。

### Phase 7: 额度重置确认与干扰池判别（F10）
...
实现通过后，版本从 `1.0.5.01` 提升到 `1.0.5.02`，同步：

- `Core/ProductIdentity.cs`
- `AGENTS.md`
- `Docs/Maintenance/CHANGELOG.jsonl`（change + deploy 记录，沿用现有 schema）
- `Docs/Interfaces/INTERFACE_INDEX.jsonl`：若已有模型目录条目则更新合并语义描述与自测入口；无则新增 `internal_api.codex_radar_model_catalog`；缓存新增键（`ContentSignature`/`CheckedAtUtc`/`Model.default.` 哨兵）在既有 radar 缓存条目中补述
- `Docs/Fable5-Data-Sources-And-Caching-Technical.md`：缓存章节补充签名持久化机制与新键说明
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\Build-Arm64.ps1 -OutputPath .\_build\DesktopCodexAssistant-arm64-model-catalog-hardening-v1.0.5.02.exe -Platform arm64
.\_build\DesktopCodexAssistant-arm64-model-catalog-hardening-v1.0.5.02.exe --test
.\_build\DesktopCodexAssistant-arm64-model-catalog-hardening-v1.0.5.02.exe --test-layout
.\_build\DesktopCodexAssistant-arm64-model-catalog-hardening-v1.0.5.02.exe --test-settings-bindings
.\_build\DesktopCodexAssistant-arm64-model-catalog-hardening-v1.0.5.02.exe --test-radar-display-lifecycle --iterations 100
```
...
| 发布 | 版本 `1.0.5.02` 三方（Release/D/E）SHA256 与版本一致，D 实例重启后 Responding |
