...
- In the shared transition Codex Radar window, Claude mode reads `ClaudeRadarReader` data instead of Codex Radar public status. It maps selected-model IQ/efficiency, community rating, and `QuotaLine` into the shared Codex Radar snapshot; IQ, quota-line, or community-rating data is enough for the refresh result to update the display. When no personal `claude-quota.ini` exists, the shared window uses Claude Radar public `quota.usage` h5/d7 values as a display fallback and suppresses the noisy `NO_SETUP_TOKEN`/legacy `NO_TOKEN` alert because that state means no setup-token source was configured, not that the visible Claude app is necessarily logged out.
