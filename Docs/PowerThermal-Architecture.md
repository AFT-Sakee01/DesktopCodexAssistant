...
| 项目 | 性能 | 平衡 | 省电 |
| --- | ---: | ---: | ---: |
| 主性能数据采样 | 500 ms | 1000 ms | 2500 ms |
| 普通面板重绘调度 | 500 ms | 1000 ms | 3000 ms |
| 悬停动画帧间隔 | 16 ms | 33 ms | 100 ms |
| 非动画交互轮询 | 30 ms | 100 ms | 250 ms |
...
| 数据 | 性能 | 平衡 | 省电 |
| --- | ---: | ---: | ---: |
| 功耗/电池采样 | 1000 ms | 2000 ms | 5000 ms |
| 正常温度采样 | 2000 ms | 5000 ms | 10000 ms |
| 温度 >= 65°C | 1500 ms | 3000 ms | 5000 ms |
| 温度 >= 70°C | 1000 ms | 2000 ms | 3000 ms |
| 温度 >= 90°C | 1000 ms | 1000 ms | 1000 ms |

温度越高，采样自动加速。严重高温时不允许省电模式把采样降到低频。
...
- WMI 从 UI 线程迁移到串行后台任务。
- 电源、电量、电源计划和有效电源模式改为事件触发刷新。
- 屏幕关闭、锁屏、挂起和全屏隐藏时暂停采样。
- 建立性能、平衡、省电三档全局与窗口级策略。
- 省电模式改为 `BelowNormal + Power Throttling`，不再使用 `Idle` 优先级。
- 温度采样根据热状态自动加速。
- 增加 70/67°C 和 95/92°C 迟滞。
- 仅在显示内容或尺寸变化时重绘。
- 重用分层窗口渲染位图，透明度动画只重新提交 Alpha。
- 无电池设备跳过 `BatteryStatus` WMI。
- 删除不可靠的电池养护暂停推断。

当时在目标 ARM64 设备上的短时测试结果约为：
