...
internal static class NativeMethods
{
    ...
    {
        IntPtr dockHandle = FindSeelenDockWindow();
        if (dockHandle == IntPtr.Zero)
        {
            detail = "Seelen dock window was not found.";
            return false;
        }
        ...
        bool success = SetWindowPos(
            ...
            SWP_SHOWWINDOW);
        detail = success
            ? "Pulsed Seelen dock window 0x" + dockHandle.ToInt64().ToString("X", CultureInfo.InvariantCulture) + " to topmost."
            : "SetWindowPos failed for Seelen dock window 0x" + dockHandle.ToInt64().ToString("X", CultureInfo.InvariantCulture) + ".";
        return success;
    }

    private static IntPtr FindSeelenDockWindow()
    {
        ...
        return preferredHandle != IntPtr.Zero ? preferredHandle : fallbackHandle;
    }
