...
internal sealed class ForegroundFpsReader : IDisposable
{
    ...
    public int? ReadForegroundFps()
    {
        ...
        bool discovered = EnsureCandidates(now);
        ...
        bool discoveryExpired =
            this.lastDiscoveryUtc != DateTime.MinValue &&
            (now - this.lastDiscoveryUtc).TotalMilliseconds >= CounterDiscoveryIntervalMs;
        if (!discovered &&
            (foregroundChanged || discoveryExpired) &&
            CanRediscoverForForegroundChange(now))
        {
    ...
    private bool EnsureCandidates(DateTime now)
    {
        if (this.lastDiscoveryUtc == DateTime.MinValue || this.candidates.Count == 0)
        {
            DiscoverCandidates(now);
            return true;
        }
